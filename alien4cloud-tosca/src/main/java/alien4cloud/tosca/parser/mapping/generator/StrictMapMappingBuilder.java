...
@Slf4j
@Component
public class StrictMapMappingBuilder implements IMappingBuilder {
    private static final String MAP = "strict_map";
    ...
    public MappingTarget buildMapping(MappingNode mappingNode, ParsingContextExecution context) {
        ...
        MappingTarget mappingTarget = new MappingTarget(map.get(MAP),
                baseParserFactory.getStrictMapParser(baseParserFactory.getReferencedParser(map.get(TYPE)), "map of " + map.get(TYPE), map.get(KEY)));
        if (map.containsKey(LIST_TYPE) && !map.get(LIST_TYPE).isEmpty()) {
            mappingTarget = new MappingTarget(map.get(MAP), baseParserFactory.getStrictMapParser(
