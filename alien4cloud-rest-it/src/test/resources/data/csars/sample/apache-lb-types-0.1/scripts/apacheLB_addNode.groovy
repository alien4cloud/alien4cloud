...
if ( configText.contains(balancerMemberText)) {
	...
	println "addNode: Added ${node} to context balancerMembers which is now ${balancerMembers} (${proxyBalancerFullPath})"
	...
	if ("${currOs}".toLowerCase().contains('windows')) {
		...
		switch (currVendor) {
			case ["Ubuntu", "Debian", "Mint"]:
				stopScript="${context.serviceDirectory}/scripts/stopOnUbuntu.sh"
				break
