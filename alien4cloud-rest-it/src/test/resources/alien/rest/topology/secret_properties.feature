Feature: Topology secrets controller
  ...
  Background:
    ...
    And I upload the archive "tosca-normative-types-1.0.0-ALIEN12"
    ...
    And I upload the archive "samples wordpress"
    And I create an application with name "secret_properties_test", archive name "secret_properties_test", description "Secret properties tests" and topology template id "wordpress-topology:2.0.0-SNAPSHOT"
  ...
  Scenario: Define a property as secret and then unset a property which is a secret
    ...
    Then I should receive a RestResponse with no error
    And The topology should contain a nodetemplate named "mysql" with property "db_password" set to "pass"
  ...
  Scenario: Define a capability as secret which is null and should receive an error
    When I define the property "port" of capability "database_endpoint" of the node "mysql" as secret with a secret path ""
    Then I should receive a RestResponse with an error code 500

  @reset
  Scenario: We can not set the property "component_version" as a secret
    When I define the property "component_version" of the node "apache" as secret with a secret path "kv/version"
    Then I should receive a RestResponse with an error code 500

  @reset
  Scenario: We can not set the capability "scalable" as a secret
    When I define the property "min_instances" of capability "scalable" of the node "computeDb" as secret with a secret path "kv/scalable"
    Then I should receive a RestResponse with an error code 500