...
public abstract class AbstractLocationResourcesSecurityController {
    ...
    public synchronized RestResponse<List<UserDTO>> grantAccessToUsers(@PathVariable String orchestratorId, @PathVariable String locationId,
        ...
        resourcePermissionService.grantPermission(resourceTemplate,
                (resource -> locationResourceService.saveResource(location, (AbstractLocationResourceTemplate) resource)), Subject.USER, userNames);
    ...
    public synchronized RestResponse<List<UserDTO>> revokeUserAccess(@PathVariable String orchestratorId, @PathVariable String locationId,
        ...
        resourcePermissionService.revokePermission(resourceTemplate,
                (resource -> locationResourceService.saveResource((AbstractLocationResourceTemplate) resource)), Subject.USER, username);
        List<UserDTO> users = UserDTO.convert(resourcePermissionService.getAuthorizedUsers(resourceTemplate));
        return RestResponseBuilder.<List<UserDTO>> builder().data(users).build();
    }
    ...
    public synchronized RestResponse<List<GroupDTO>> grantAccessToGroups(@PathVariable String orchestratorId, @PathVariable String locationId,
        ...
        AbstractLocationResourceTemplate resourceTemplate = locationResourceService.getOrFail(resourceId);
        resourcePermissionService.grantPermission(resourceTemplate,
                (resource -> locationResourceService.saveResource(location, (AbstractLocationResourceTemplate) resource)), Subject.GROUP, groupIds);
    ...
    public synchronized RestResponse<List<GroupDTO>> revokeGroupAccess(@PathVariable String orchestratorId, @PathVariable String locationId,
        ...
        AbstractLocationResourceTemplate resourceTemplate = locationResourceService.getOrFail(resourceId);
        resourcePermissionService.revokePermission(resourceTemplate,
                (resource -> locationResourceService.saveResource((AbstractLocationResourceTemplate) resource)), Subject.GROUP, groupId);
        List<GroupDTO> groups = GroupDTO.convert(resourcePermissionService.getAuthorizedGroups(resourceTemplate));
        return RestResponseBuilder.<List<GroupDTO>> builder().data(groups).build();
    }
    ...
    public synchronized RestResponse<Void> revokeApplicationAccess(@PathVariable String orchestratorId, @PathVariable String locationId,
        ...
        AbstractLocationResourceTemplate resourceTemplate = locationResourceService.getOrFail(resourceId);
        resourcePermissionService.revokePermission(resourceTemplate,
                (resource -> locationResourceService.saveResource((AbstractLocationResourceTemplate) resource)), Subject.APPLICATION, applicationId);
        ...
        resourcePermissionService.revokePermission(resourceTemplate,
                (resource -> locationResourceService.saveResource((AbstractLocationResourceTemplate) resource)), Subject.ENVIRONMENT, envIds);
        ...
        resourcePermissionService.revokePermission(resourceTemplate,
                (resource -> locationResourceService.saveResource((AbstractLocationResourceTemplate) resource)), Subject.ENVIRONMENT_TYPE,
                envTypeIds.toArray(new String[envTypeIds.size()]));
    ...
    @PreAuthorize("hasAuthority('ADMIN')")
    public synchronized RestResponse<Void> updateAuthorizedEnvironmentsAndEnvTypePerApplication(@PathVariable String orchestratorId,
            @PathVariable String locationId, @PathVariable String resourceId, @RequestBody ApplicationEnvironmentAuthorizationUpdateRequest request) {
        Location location = locationService.getLocation(orchestratorId, locationId);
        ...
        if (ArrayUtils.isNotEmpty(request.getEnvironmentTypesToAdd())) {
            resourcePermissionService.grantPermission(resourceTemplate,
                    (resource -> locationResourceService.saveResource(location, (AbstractLocationResourceTemplate) resource)), Subject.ENVIRONMENT_TYPE,
                    request.getEnvironmentTypesToAdd());
        }
        return RestResponseBuilder.<Void> builder().build();
    }
    ...
    public RestResponse<List<ApplicationEnvironmentAuthorizationDTO>> getAuthorizedEnvironmentsAndEnvTypePerApplication(@PathVariable String orchestratorId,
        ...
        if (MapUtils.isNotEmpty(resourceTemplate.getEnvironmentPermissions())) {
            ...
            applicationsRelatedToEnvironment = alienDAO.findByIds(Application.class,
        ...
        if (MapUtils.isNotEmpty(resourceTemplate.getEnvironmentTypePermissions())) {
            ...
            applicationsRelatedToEnvironmentType = alienDAO.findByIds(Application.class,
                    environmentTypeApplicationIds.toArray(new String[environmentTypeApplicationIds.size()]));
        }
        ...
        List<ApplicationEnvironmentAuthorizationDTO> result = ApplicationEnvironmentAuthorizationDTO.buildDTOs(applicationsRelatedToEnvironment,
                applicationsRelatedToEnvironmentType, environments, applications, environmentTypes);
