...
public class CsarGithubService {
    ...
    public ParsingResult<Csar>[] specifyCsarFromGit(String param) throws CSARVersionAlreadyExistsException, ParsingException {
        ...
        List<String> branchs = new LinkedList<String>();
        List<CsarGitCheckoutLocation> locationsList;
        RepositoryManager repoManager = new RepositoryManager();
        Path alienTpmPath = Paths.get(alienTempUpload);
        if (csarGit == null) {
            throw new NotFoundException("CsarGit " + "[" + data + "] doesn't exist");
        }
        locationsList = csarGit.getImportLocations();
        if (locationsList.isEmpty()) {
            throw new NotFoundException("CsarGit " + "[" + data + "] doesn't contain any import locations");
        }
        for (CsarGitCheckoutLocation location : csarGit.getImportLocations()) {
            branchs.add(location.getBranchId());
        }
        repoManager.createFolderAndCloneAndZip(alienTpmPath, csarGit.getRepositoryUrl(), branchs, _LOCALDIRECTORY);
