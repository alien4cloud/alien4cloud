Feature: Topology editor: remove group member operation
  ...
  Scenario: Removing a node from a group when the node was the only member should succeed
    ...
    And The topology SPEL expression "nodeTemplates['Compute'].groups.size()" should return "0"
  ...
  Scenario: Removing a node from a group that has other member should succeed
    ...
    Then No exception should be thrown
    And The topology SPEL int expression "groups.size()" should return 1
    And The topology SPEL expression "groups['simple_group'].members[0]" should return "Compute_2"
    And The topology SPEL expression "nodeTemplates['Compute'].groups.size()" should return "0"
    And The topology SPEL expression "nodeTemplates['Compute_2'].groups[0]" should return "simple_group"
