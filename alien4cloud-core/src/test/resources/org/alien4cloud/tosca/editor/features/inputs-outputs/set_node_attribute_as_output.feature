Feature: Topology editor: set node attribute as output
  ...
  Scenario: Set a node attribute as output should succeed
    ...
    And The SPEL expression "outputAttributes['network'].size()" should return 1
    And The SPEL expression "outputAttributes['network'].?[#this == 'tosca_name'].size()" should return 1
    When I execute the operation
      | type          | org.alien4cloud.tosca.editor.operations.nodetemplate.outputs.SetNodeAttributeAsOutputOperation |
      | nodeName      | network                                                                                        |
    ...
    Then No exception should be thrown
    And The SPEL expression "outputAttributes.size()" should return 1
    And The SPEL expression "outputAttributes['network'].size()" should return 2
    And The SPEL expression "outputAttributes['network'].?[#this == 'tosca_id'].size()" should return 1
