Feature: Topology editor: Recover a topology after csar dependencies updates
  ...
  Scenario: Delete a node type from archive and recover the topology
    ...
    Then The SPEL int expression "operation.updatedDependencies.size()" should return 1
    And The SPEL int expression "operation.updatedDependencies.?[#this.name == 'test-topo-recovery-types' and #this.version == '0.1-SNAPSHOT' ].size()" should return 1
