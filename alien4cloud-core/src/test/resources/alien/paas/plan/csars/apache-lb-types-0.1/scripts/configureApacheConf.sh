...
do    
	if [ -d "$i" ] ; then
		...
		if [ -f "${currConf}" ] ; then
			...
			echo "Writing ${proxyBalancerPath} location to ${serviceDirectory}/scripts/proxyBalancerPath ..."
			sudo echo "pathToBalancerConf=\"${proxyBalancerPath}\"" > ${serviceDirectory}/scripts/proxyBalancerPath || error_exit $? "Failed on: sudo echo to ${serviceDirectory}/scripts/proxyBalancerPath"
			sudo chmod 777 ${serviceDirectory}/scripts/proxyBalancerPath || error_exit $? "Failed on: 	sudo chmod 777 ${serviceDirectory}/scripts/proxyBalancerPath "	
			exit 0
		fi	
    fi	
done
