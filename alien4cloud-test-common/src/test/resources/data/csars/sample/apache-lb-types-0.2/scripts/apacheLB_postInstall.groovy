...
switch (currVendor) {
	case ["Ubuntu", "Debian", "Mint"]:
		confScript="${context.serviceDirectory}/scripts/configureApacheConfOnUbuntu.sh"
		origProxyBalancerPath="${context.serviceDirectory}/scripts/overrides-linux/conf/ubuntu"
...
if ( isLinux ) {
	builder.sequential {
		...
		exec(executable: "${confScript}",failonerror: "true") {
			...
			arg(value:"${context.serviceDirectory}/scripts")
		}
	}
}
...
if( !isLinux ) {
	...
	context.attributes.thisInstance["proxyBalancerPath"] = "${context.serviceDirectory}/scripts/install/conf/extra/${proxyBalancerName}"
	proxyConfigFile = new File("${context.serviceDirectory}/scripts/install/conf/extra/${proxyBalancerName}")
